...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_correction_v1_${TIMESTAMP}}"
...
if [[ "$SKIP_TRAIN" != "1" ]]; then
  echo "[phase2_slot_correction_v1] training diagnostic_adapter_slot_correction"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/train_sft.py" \
    --config "$DIAGNOSTIC_CONFIG" \
    --run-root "$RUN_ROOT" \
    --verbose
else
  echo "[phase2_slot_correction_v1] skipping training"
fi
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction_v1] running standalone diagnostic held-out compare"
  ...
  echo "[phase2_slot_correction_v1] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction_v1] running oracle/predicted slot-correction forecast integration compare"
  ...
  echo "[phase2_slot_correction_v1] skipping forecast integration compare"
fi

echo "[phase2_slot_correction_v1] completed"
