#!/usr/bin/env python3
"""Preflight validation for Phase 2 track-correction oracle gate."""
...
DIAGNOSTIC_VIEW_ROOT = (DATASET_ROOT / "views" / "diagnostic_track_correction_only").resolve()
...
def _validate_dataset_ready(errors: list[str]) -> None:
    ...
    _require(
        bool(payload.get("diagnostic_track_correction_view_ready")),
        (
            "dataset_ready_report must mark diagnostic_track_correction_view_ready=true: "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Validate Phase 2 track-correction oracle gate configs and artifacts."
    )
    ...
    print(f"OK: diagnostic_track_correction_view={DIAGNOSTIC_VIEW_ROOT}")
