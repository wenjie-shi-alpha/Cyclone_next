...
DIAGNOSTIC_CONFIG="${DIAGNOSTIC_CONFIG:-$ROOT_DIR/configs/training/sft_gemma_4_e4b_unsloth_diagnostic_v2_slot_correction_stage_v0.yaml}"
...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_correction_v0_${TIMESTAMP}}"
...
if [[ "$SKIP_TRAIN" != "1" ]]; then
  echo "[phase2_slot_correction] training diagnostic_adapter_slot_correction"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/train_sft.py" \
    --config "$DIAGNOSTIC_CONFIG" \
    --run-root "$RUN_ROOT" \
    --verbose
else
  echo "[phase2_slot_correction] skipping training"
fi
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction] running standalone diagnostic held-out compare"
  ...
  echo "[phase2_slot_correction] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction] running oracle/predicted slot-correction forecast integration compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_slot_correction_forecast_integration.py" \
    ...
    --predicted-label "predicted_slot_locked_forecast_correction_v0" \
    --official-label "expert_official" \
    --output "$FORECAST_OUTPUT"
else
  echo "[phase2_slot_correction] skipping forecast integration compare"
fi

echo "[phase2_slot_correction] completed"
