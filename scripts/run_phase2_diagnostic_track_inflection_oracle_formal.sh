...
DIAGNOSTIC_TRAIN_DATASET="${DIAGNOSTIC_TRAIN_DATASET:-$DATASET_ROOT/views/diagnostic_track_inflection_only/train.jsonl}"
DIAGNOSTIC_TEST_DATASET="${DIAGNOSTIC_TEST_DATASET:-$DATASET_ROOT/views/diagnostic_track_inflection_only/test.jsonl}"
...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_track_inflection_oracle_v0_${TIMESTAMP}}"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/check_phase2_diagnostic_track_inflection_oracle.py" \
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2_track_inflection_oracle] running standalone diagnostic synthetic baselines"
  ...
  echo "[phase2_track_inflection_oracle] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2_track_inflection_oracle] running oracle diagnostic forecast integration compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_diagnostic_forecast_integration.py" \
    ...
    --section-title "Track-Inflection Candidate v0 Assessment" \
    --baseline-label "baseline_forecast_sft_v2" \
    --oracle-label "oracle_track_inflection_candidate_v0_plus_forecast" \
    --official-label "expert_official" \
    --skip-predicted \
    --output "$FORECAST_OUTPUT"
else
  echo "[phase2_track_inflection_oracle] skipping forecast integration compare"
fi

echo "[phase2_track_inflection_oracle] completed"
