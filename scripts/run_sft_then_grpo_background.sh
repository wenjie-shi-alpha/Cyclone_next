...
DEFAULT_SFT_CONFIG="$ROOT_DIR/configs/training/sft_gemma_4_e4b_unsloth_forecast_v2_round1.yaml"
DEFAULT_GRPO_CONFIG="$ROOT_DIR/configs/training/grpo_gemma_4_e4b_unsloth_forecast_v2_round1.yaml"
PYTHON_BIN="$ROOT_DIR/.venv/bin/python"

SFT_CONFIG="${1:-$DEFAULT_SFT_CONFIG}"
GRPO_CONFIG="${2:-$DEFAULT_GRPO_CONFIG}"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/sft_grpo_${TIMESTAMP}}"
...
if [[ ! -f "$SFT_CONFIG" ]]; then
  echo "Missing SFT config: $SFT_CONFIG" >&2
  exit 1
fi
...
if [[ "${DISABLE_TMUX:-0}" != "1" ]] && command -v tmux >/dev/null 2>&1; then
  BACKEND="tmux"
  SESSION_NAME="${TMUX_SESSION_NAME:-cyclone_train_${TIMESTAMP}}"
