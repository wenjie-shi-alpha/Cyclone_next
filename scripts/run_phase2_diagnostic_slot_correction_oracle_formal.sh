...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_correction_oracle_v0_${TIMESTAMP}}"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/check_phase2_diagnostic_slot_correction_oracle.py" \
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction_oracle] running standalone diagnostic synthetic baselines"
  ...
  echo "[phase2_slot_correction_oracle] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction_oracle] running slot-locked oracle forecast integration compare"
  ...
  echo "[phase2_slot_correction_oracle] skipping forecast integration compare"
fi

echo "[phase2_slot_correction_oracle] completed"
