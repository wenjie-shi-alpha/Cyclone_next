#!/usr/bin/env python3
"""Preflight validation for Phase 2 track-turn diagnostic closure."""
...
DIAGNOSTIC_VIEW_ROOT = (DATASET_ROOT / "views" / "diagnostic_track_turn_only").resolve()
...
def _validate_dataset_ready(errors: list[str]) -> None:
    ...
    _require(
        bool(payload.get("diagnostic_track_turn_view_ready")),
        f"dataset_ready_report must mark diagnostic_track_turn_view_ready=true: {DATASET_READY_REPORT}",
        errors,
    )
...
def _validate_diagnostic_sft(config_path: Path, errors: list[str]) -> str | None:
    ...
    _require(
        ...
        (
            "Diagnostic train file must use the diagnostic_track_turn_only train split: "
    ...
    _require(
        ...
        (
            "Diagnostic eval file must use the diagnostic_track_turn_only val split: "
    ...
    _require(
        ...
        "Track-turn staged SFT must set trainer.adapter_init_path.",
        errors,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Phase 2 track-turn diagnostic closure configs and artifacts.")
    parser.add_argument("--diagnostic-config", required=True, help="Path to the track-turn diagnostic SFT YAML config.")
    ...
    print(f"OK: diagnostic_track_turn_view={DIAGNOSTIC_VIEW_ROOT}")
