...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_turn_correction_v0_confirm_${TIMESTAMP}}"
...
cmd=(
  ...
  SOURCE_RUN_ROOT="${SOURCE_RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_turn_correction_v0_20260419_180019}"
  DIAGNOSTIC_ADAPTER="${DIAGNOSTIC_ADAPTER:-$ROOT_DIR/runs/phase2_diagnostic_slot_turn_correction_v0_20260419_180019/sft/final_adapter}"
  ...
  DIAGNOSTIC_TRAIN_DATASET="${DIAGNOSTIC_TRAIN_DATASET:-$ROOT_DIR/data/training_rebuilt_v2_20260414_guidancefix/views/diagnostic_slot_turn_correction_only/train.jsonl}"
  DIAGNOSTIC_TEST_DATASET="${DIAGNOSTIC_TEST_DATASET:-$ROOT_DIR/data/training_rebuilt_v2_20260414_guidancefix/views/diagnostic_slot_turn_correction_only/test.jsonl}"
  ...
  bash
  "$ROOT_DIR/scripts/run_phase2_diagnostic_slot_turn_correction_v0_confirm_formal.sh"
)
...
if [[ "${DISABLE_TMUX:-0}" != "1" ]] && command -v tmux >/dev/null 2>&1; then
  BACKEND="tmux"
  SESSION_NAME="${TMUX_SESSION_NAME:-phase2_diag_slot_turn_confirm_${TIMESTAMP}}"
