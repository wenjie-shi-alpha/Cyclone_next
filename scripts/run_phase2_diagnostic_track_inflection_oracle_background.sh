...
PID_FILE="$RUN_ROOT/oracle_gate.pid"
...
cmd=(
  ...
  DIAGNOSTIC_TRAIN_DATASET="${DIAGNOSTIC_TRAIN_DATASET:-$ROOT_DIR/data/training_rebuilt_v2_20260414_guidancefix/views/diagnostic_track_inflection_only/train.jsonl}"
  DIAGNOSTIC_TEST_DATASET="${DIAGNOSTIC_TEST_DATASET:-$ROOT_DIR/data/training_rebuilt_v2_20260414_guidancefix/views/diagnostic_track_inflection_only/test.jsonl}"
  ...
  bash
  "$ROOT_DIR/scripts/run_phase2_diagnostic_track_inflection_oracle_formal.sh"
)
...
if [[ "${DISABLE_TMUX:-0}" != "1" ]] && command -v tmux >/dev/null 2>&1; then
  BACKEND="tmux"
  SESSION_NAME="${TMUX_SESSION_NAME:-phase2_diag_track_inflection_oracle_${TIMESTAMP}}"
