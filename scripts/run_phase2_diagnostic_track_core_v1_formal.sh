...
DIAGNOSTIC_CONFIG="${DIAGNOSTIC_CONFIG:-$ROOT_DIR/configs/training/sft_gemma_4_e4b_unsloth_diagnostic_v2_track_core_v1.yaml}"
...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_track_core_v1_${TIMESTAMP}}"
...
if [[ "$SKIP_TRAIN" != "1" ]]; then
  echo "[phase2c] training diagnostic_adapter_track_core_v1"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/train_sft.py" \
    --config "$DIAGNOSTIC_CONFIG" \
    --run-root "$RUN_ROOT" \
    --verbose
else
  echo "[phase2c] skipping training"
fi
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2c] running standalone diagnostic held-out compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_diagnostic_models.py" \
    ...
    --model "diagnostic_adapter_track_core_v1=$DIAGNOSTIC_ADAPTER" \
  ...
  echo "[phase2c] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2c] running oracle/predicted diagnostic forecast integration compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_diagnostic_forecast_integration.py" \
    ...
    --oracle-label "oracle_track_core_diagnostics_plus_forecast_v1" \
    --predicted-label "predicted_track_core_diagnostics_plus_forecast_v1" \
    --official-label "expert_official" \
    --output "$FORECAST_OUTPUT"
else
  echo "[phase2c] skipping forecast integration compare"
fi

echo "[phase2c] completed"
