...
cmd=(
  ...
  bash
  "$ROOT_DIR/scripts/run_phase2_slot_correction_intensity_gate_v0_formal.sh"
)
...
if [[ "${DISABLE_TMUX:-0}" != "1" ]] && command -v tmux >/dev/null 2>&1; then
  BACKEND="tmux"
  SESSION_NAME="${TMUX_SESSION_NAME:-phase2_slot_correction_intensity_gate_v0_${TIMESTAMP}}"
