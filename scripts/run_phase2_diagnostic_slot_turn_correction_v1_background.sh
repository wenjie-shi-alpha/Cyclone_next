...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_turn_correction_v1_${TIMESTAMP}}"
...
cmd=(
  ...
  bash
  "$ROOT_DIR/scripts/run_phase2_diagnostic_slot_turn_correction_v1_formal.sh"
)
...
if [[ "${DISABLE_TMUX:-0}" != "1" ]] && command -v tmux >/dev/null 2>&1; then
  BACKEND="tmux"
  SESSION_NAME="${TMUX_SESSION_NAME:-phase2_diag_slot_turn_correction_v1_${TIMESTAMP}}"
