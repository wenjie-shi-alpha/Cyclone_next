#!/usr/bin/env python3
"""Preflight validation for Phase 2 slot-locked correction oracle gate."""
...
def _validate_slot_correction_files(errors: list[str]) -> None:
    for split_name, path in EXPECTED_DIAGNOSTIC_PATHS.items():
        _require_file(path, f"Slot-correction diagnostic {split_name} split", errors)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Validate Phase 2 slot-locked correction oracle gate configs and artifacts."
    )
    ...
    _validate_slot_correction_files(errors)
