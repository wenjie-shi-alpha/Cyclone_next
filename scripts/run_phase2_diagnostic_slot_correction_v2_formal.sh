...
DIAGNOSTIC_CONFIG="${DIAGNOSTIC_CONFIG:-$ROOT_DIR/configs/training/sft_gemma_4_e4b_unsloth_diagnostic_v2_slot_correction_stage_v2.yaml}"
...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_correction_v2_${TIMESTAMP}}"
...
echo "ORACLE_REPORT=$ORACLE_REPORT"

if [[ "$SKIP_TRAIN" != "1" ]]; then
  echo "[phase2_slot_correction_v2] training diagnostic_adapter_slot_correction"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/train_sft.py" \
    --config "$DIAGNOSTIC_CONFIG" \
    --run-root "$RUN_ROOT" \
    --verbose
else
  echo "[phase2_slot_correction_v2] skipping training"
fi
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction_v2] running standalone diagnostic held-out compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_diagnostic_models.py" \
    ...
    --model "diagnostic_adapter_slot_correction=$DIAGNOSTIC_ADAPTER" \
  ...
  echo "[phase2_slot_correction_v2] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2_slot_correction_v2] running oracle/predicted slot-correction forecast integration compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_slot_correction_forecast_integration.py" \
    ...
    --oracle-label "oracle_slot_locked_forecast_correction_v0" \
    --predicted-label "predicted_slot_locked_forecast_correction_v2" \
    --official-label "expert_official" \
    --output "$FORECAST_OUTPUT"
else
  echo "[phase2_slot_correction_v2] skipping forecast integration compare"
fi

echo "[phase2_slot_correction_v2] completed"
