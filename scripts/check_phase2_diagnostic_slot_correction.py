#!/usr/bin/env python3
"""Preflight validation for Phase 2 slot-correction predicted training/closure."""
...
DIAGNOSTIC_VIEW_ROOT = (DATASET_ROOT / "views" / "diagnostic_slot_correction_only").resolve()
...
DEFAULT_SUCCESSFUL_ORACLE_REPORT = (
    ROOT
    / "runs/phase2_diagnostic_slot_correction_oracle_v0_20260418_154645/evals/forecast_integration_compare_sample200_seed3407.json"
).resolve()
...
def _validate_dataset_ready(errors: list[str]) -> None:
    ...
    _require(
        bool(payload.get("diagnostic_slot_correction_view_ready")),
        (
            "dataset_ready_report must mark diagnostic_slot_correction_view_ready=true: "
...
def _validate_oracle_gate(oracle_report_path: Path, errors: list[str]) -> None:
    _require_file(oracle_report_path, "slot-correction oracle report", errors)
    if errors:
        return
    payload = json.loads(oracle_report_path.read_text(encoding="utf-8"))
    summary_rows = {
        str(row.get("label")): row
        for row in payload.get("summary", []) or []
    }
    baseline = summary_rows.get("baseline_forecast_sft_v2")
    oracle = summary_rows.get("oracle_slot_locked_forecast_correction_v0")
    _require(
        baseline is not None,
        f"Oracle report missing baseline_forecast_sft_v2 summary row: {oracle_report_path}",
        errors,
    )
    _require(
        oracle is not None,
        f"Oracle report missing oracle_slot_locked_forecast_correction_v0 summary row: {oracle_report_path}",
        errors,
    )
    if baseline is None or oracle is None:
        return
    _require(
        float(oracle["track_error_km"]) < float(baseline["track_error_km"]),
        (
            "Predicted training is gated on a passed slot-correction oracle run: "
            f"oracle track_error_km must improve baseline in {oracle_report_path}"
        ),
        errors,
    )
    _require(
        float(oracle["mean_track_diff_vs_official_km"]) < float(baseline["mean_track_diff_vs_official_km"]),
        (
            "Predicted training is gated on a passed slot-correction oracle run: "
            f"oracle mean_track_diff_vs_official_km must improve baseline in {oracle_report_path}"
        ),
        errors,
    )
...
def _validate_diagnostic_sft(config_path: Path, errors: list[str]) -> str | None:
    ...
    _require(
        ...
        (
            "Diagnostic train file must use the diagnostic_slot_correction_only train split: "
    ...
    _require(
        ...
        (
            "Diagnostic eval file must use the diagnostic_slot_correction_only val split: "
    ...
    _require(
        ...
        "Slot-correction predicted SFT should use staged init from an existing diagnostic adapter.",
        errors,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Phase 2 slot-correction predicted training configs and artifacts.")
    parser.add_argument("--diagnostic-config", required=True, help="Path to the slot-correction diagnostic SFT YAML config.")
    ...
    parser.add_argument(
        "--oracle-report",
        default=str(DEFAULT_SUCCESSFUL_ORACLE_REPORT),
        help="Successful oracle compare report gating predicted training.",
    )
    ...
    _validate_oracle_gate(Path(args.oracle_report).resolve(), errors)
    ...
    print(f"OK: diagnostic_slot_correction_view={DIAGNOSTIC_VIEW_ROOT}")
    print(f"OK: forecast_view={FORECAST_VIEW_ROOT}")
    print(f"OK: canonical_root={CANONICAL_ROOT}")
    print(f"OK: oracle_report={Path(args.oracle_report).resolve()}")
