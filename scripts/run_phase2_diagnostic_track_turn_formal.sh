...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_track_turn_v0_${TIMESTAMP}}"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/check_phase2_diagnostic_track_turn.py" \
...
if [[ "$SKIP_TRAIN" != "1" ]]; then
  echo "[phase2_track_turn] training diagnostic_adapter_track_turn"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/train_sft.py" \
    --config "$DIAGNOSTIC_CONFIG" \
    --run-root "$RUN_ROOT" \
    --verbose
else
  echo "[phase2_track_turn] skipping training"
fi
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2_track_turn] running standalone diagnostic held-out compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_diagnostic_models.py" \
    ...
    --model "diagnostic_adapter_track_turn=$DIAGNOSTIC_ADAPTER" \
  ...
  echo "[phase2_track_turn] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2_track_turn] running oracle/predicted diagnostic forecast integration compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_diagnostic_forecast_integration.py" \
    ...
    --oracle-label "oracle_track_turn_diagnostics_plus_forecast" \
    --predicted-label "predicted_track_turn_diagnostics_plus_forecast" \
    --official-label "expert_official" \
    --output "$FORECAST_OUTPUT"
else
  echo "[phase2_track_turn] skipping forecast integration compare"
fi

echo "[phase2_track_turn] completed"
