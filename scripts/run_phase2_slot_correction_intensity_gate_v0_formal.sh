...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_slot_correction_intensity_gate_v0_${TIMESTAMP}}"
...
"$PYTHON_BIN" "$ROOT_DIR/scripts/compare_slot_correction_forecast_integration.py" \
  ...
  --oracle-baseline-intensity-label "oracle_slot_locked_track_plus_baseline_intensity_v0" \
  --predicted-baseline-intensity-label "predicted_slot_locked_track_plus_baseline_intensity_v0" \
  --official-label "expert_official" \
  --output "$OUTPUT"

echo "[phase2_slot_correction_intensity_gate_v0] completed"
