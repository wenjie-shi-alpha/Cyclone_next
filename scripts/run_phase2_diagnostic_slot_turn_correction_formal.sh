...
DIAGNOSTIC_CONFIG="${DIAGNOSTIC_CONFIG:-$ROOT_DIR/configs/training/sft_gemma_4_e4b_unsloth_diagnostic_v2_slot_turn_correction_stage_v0.yaml}"
...
RUN_ROOT="${RUN_ROOT:-$ROOT_DIR/runs/phase2_diagnostic_slot_turn_correction_v0_${TIMESTAMP}}"
...
if [[ "$SKIP_TRAIN" != "1" ]]; then
  echo "[phase2c_slot_turn] training diagnostic_adapter_slot_turn_correction"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/train_sft.py" \
    --config "$DIAGNOSTIC_CONFIG" \
    --run-root "$RUN_ROOT" \
    --verbose
else
  echo "[phase2c_slot_turn] skipping training"
fi
...
if [[ "$SKIP_STANDALONE_COMPARE" != "1" ]]; then
  echo "[phase2c_slot_turn] running standalone diagnostic held-out compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_diagnostic_models.py" \
    ...
    --model "diagnostic_adapter_slot_turn_correction=$DIAGNOSTIC_ADAPTER" \
  ...
  echo "[phase2c_slot_turn] skipping standalone diagnostic compare"
fi

if [[ "$SKIP_FORECAST_COMPARE" != "1" ]]; then
  echo "[phase2c_slot_turn] running oracle/predicted slot-turn forecast integration compare"
  "$PYTHON_BIN" "$ROOT_DIR/scripts/compare_slot_correction_forecast_integration.py" \
    ...
    --oracle-label "oracle_slot_turn_locked_forecast_correction_v0" \
    --predicted-label "predicted_slot_turn_locked_forecast_correction_v0" \
    --oracle-baseline-intensity-label "oracle_slot_turn_track_plus_baseline_intensity_scale_1p20_v0" \
    --predicted-baseline-intensity-label "predicted_slot_turn_track_plus_baseline_intensity_scale_1p20_v0" \
    --official-label "expert_official" \
    --baseline-intensity-offset-scale "$BASELINE_INTENSITY_OFFSET_SCALE" \
    --output "$FORECAST_OUTPUT"
else
  echo "[phase2c_slot_turn] skipping forecast integration compare"
fi

echo "[phase2c_slot_turn] completed"
